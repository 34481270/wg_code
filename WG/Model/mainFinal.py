...
def command_router(cmd_q, shutdown_evt, shared, sender):
    while not shutdown_evt.is_set():
        ...
        if cmd == Cmd.START:
            ...
            sender.sendQueue.put(('State', '停止動作'))
            ...
            shared['prop_enabled'].value = 1
            sender.sendQueue.put(('State', '依設定執行固定舵角與航速'))
...
def rudderPidTest(shared, shutdown_evt):
    ...
    while not shutdown_evt.is_set():
        ...
        if now - last_rudder_update_time >= shared['rudder_update_interval'].value:
            ...
            # 指令與當前航向
            cmd = shared['pidTestCourseAngle'].value
            
            course_angle = ( 90 - shared['IMU_Glider'].get('angle_yaw', 0.0)) % 360
        ...
        while (not shutdown_evt.is_set()) and (shared['nav_enabled'].value == 1):
            ...
            if now - last_rudder_update_time >= shared['rudder_update_interval'].value:
                ...
                if shared['reset_simulation'].value:
                    ...
                    # ---把模擬＆GPS路徑轉為UTM---
                    dubins_path_latlon = list(shared['dubins_path'])  # [[lat, lon], ...]
                    # 轉成 UTM
                    dubins_path_utm = [
                        list(gps_utils.gps_to_utm(lat, lon))
                        for lat, lon in dubins_path_latlon
                    ]

                    
                    closest_pt1 = find_nearest_projection(np.array([x0, y0]), np.array(dubins_path_utm)) #應該要改成current_path吧？
                    closest_pt2 = find_nearest_projection(np.array(gps_utils.gps_to_utm(lat, lon)), np.array(dubins_path_utm))
                    print("------------------")
                    print(closest_pt1)
                    print(closest_pt2)
                    print("------------------")
                    ...
                    shared['simu_sender_queue'].put(("drawOrigin", [[simu_lat, simu_lon], [lat, lon], gps_utils.utm_to_gps(closest_pt1[0], closest_pt1[1]), gps_utils.utm_to_gps(closest_pt2[0], closest_pt2[1])]))
                    ...
                    Uc_est, Vc_est, P_new, V = KalmanEstimateCurrent.KalmanEstimateCurrent(
                        ...
                        np.array([closest_pt1, closest_pt2]),
                    ...
                    tmp_current_speed = np.hypot(shared['Uc'].value, shared['Vc'].value)
                    tmp_current_angle = (90 - np.degrees(np.arctan2(shared['Vc'].value, shared['Uc'].value))) % 360
                    # print('tmp_current_angle = ', tmp_current_angle)
                    # print('tmp_current_angle = ', (90-tmp_current_angle)%360)
                    
                    shared['current_speed'].value = tmp_current_speed
                    shared['current_angle'].value = tmp_current_angle 
                    
                    closest2_lat, closest2_lon = gps_utils.utm_to_gps(closest_pt2[0], closest_pt2[1])
                    shared['simu_sender_queue'].put(("drawArrow", [closest2_lat, closest2_lon, tmp_current_angle, tmp_current_speed]))
...
def pathPlanningProcess(shared): 
    ...
    if len(goal) < 2: # 如果沒有設定goal就return 不規劃路徑
        return

    # 每次路徑規劃時先把當下的載具位置存下來 讓之後估測流場時投影使用
    shared['simu_gps']['lat'] = gps.get('lat', 0.0)
    shared['simu_gps']['lon'] = gps.get('lon', 0.0)
    ...
    heading = shared['IMU_Glider'].get('angle_yaw', 0.0)
    ...
    # 轉回絕對 UTM 再轉 GPS 經緯度
    if dubins_path is None or current_path is None:
        return 1
...
def process_queue_data(shared, sender, cmd_queue):
    ...
    def pid_parameter_adjust(data):
        ...
        if data[0] == 'propeller':
            ...
            shared['pidTestCourseAngle'].value = data[6]
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    print("reset buffers err:", repr(e))
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                print(f"? Read error (likely disconnected): {repr(e)}")
                try: self.ser.close()
                except: pass
                self.ser = None
                time.sleep(self.retry_interval)
                ...
                print(f"Unexpected read error: {repr(e)}")
                time.sleep(0.05)
                continue
...
class ArduinoReceive(threading.Thread):
    ...
    def connect_serial(self):
        while self.running:
            try:
                self.safe_close()
                # ????? DTR??????
                self.ser = serial.Serial(self.port, self.baudrate, timeout=1, write_timeout=1, dsrdtr=False, rtscts=False)
                print("[Receiver] open ok")
                ...
                try:
                    ...
                    print("[Receiver] reset buffers err:", repr(e))

                print("FloaterArduino ready")
                return
            except serial.SerialException as e:
                ...
                print("[Receiver] connect_serial unexpected:", repr(e))
                traceback.print_exc()
                self.safe_close()
                time.sleep(self.retry_interval)
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                raw = self.ser.readline()  # ??? timeout=1 ?
                if not raw:
                    time.sleep(0.01)
                    continue
                ...
                try:
                    ...
                    print("[Receiver] decode err:", repr(de))
                    continue
                ...
                print(f"[Receiver] IO error: {repr(e)} -> reconnect")
                self.safe_close()
                time.sleep(self.retry_interval)
            except Exception as e:
                print("[Receiver] unexpected:", repr(e))
                traceback.print_exc()
    ...
    def handle_command(self, cmd, data):
        try:
            if cmd == "BatteryVoltage":
                ...
                print(f"[Receiver] unknown: {cmd}@{data}")
        except ValueError:
            print(f"[Receiver] value error: {cmd}@{data}")
...
def main():
    ...
    shared = {
        ...
        'pidTestCourseAngle': manager.Value('d', 0),
        ...
        'R': manager.Value('d', 30.0),
        'goal':  manager.list([]), 
        'replan_dis': manager.Value('d', 50.0),
    ...
    nav_process.start()
    # floaterMainArduino = ArduinoReceive(port="/dev/FloaterArduino", baudrate=9600, shared=shared)
    ...
    while True:
            ...
            stats = {
                ...
                'heading':  (90 - shared['IMU_Glider'].get('angle_yaw', 0.0)) % 360,
                ...
                'propeller' : shared['propellerParameter'].value,
            }
            ...
            if path_version > last_path_sent_version:
                sender.sendQueue.put(("drawCurrentPathTrajectory", list(shared['current_path'])))
                sender.sendQueue.put(("drawDubinsPathTrajectory", list(shared['dubins_path'])))
                shared['last_path_sent_version'].value += 1     
